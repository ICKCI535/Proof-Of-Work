...
def start_match_bet(expired, spread, event_id, reward_pool, web3: Web3, contract: Contract, caller_pk: str, caller_address: str):
    nonce = web3.eth.get_transaction_count(caller_address)
    txn = contract.functions.startMatchBet(expired, spread, event_id, reward_pool).buildTransaction(
        {
            "from": caller_address,
            "gas": 2000000,
            "gasPrice": web3.eth.gas_price,
...
if __name__ == "__main__":
    address = Web3.toChecksumAddress(
        "0x6cF50FeDDeE66089191AA1F940d3F565C0cf9Cf5")
    ...
    print(schedule)
    start_match_bet(expired=schedule['event_dates'], spread=schedule['point_spread_homes'], event_id=schedule['event_ids'], reward_pool=schedule['reward_pools'], web3=web3, contract=contract,
